...
object Registry extends DefaultUserRepositoryComponent
  ...
  val matt = userFactory.create( "mkemnetz@luc.edu", "password", "Matthew", "Kemnetz", new Role(false, true, true, true))
  userRepository.addUser(matt)
  val pat = userFactory.create("pbehrens@luc.edu", "password", "Pat", "Behrens", new Role(false, true, true, true))
